...
export function SkillsGallery({ initialSkills, initialCategory, initialTag, locale = 'en' }: SkillsGalleryProps) {
  const [filterType] = useState<FilterType>("all")
  const [searchQuery, setSearchQuery] = useState("")
  const [category, setCategory] = useState(initialCategory || "all")
  const [tag, setTag] = useState(initialTag)

  // Track URL changes for filters
  useEffect(() => {
    ...
    // Update on initial load and URL changes
        ...
        <div className="container mx-auto px-4 py-6">
          ...
          {(category !== "all" || tag) && (
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              {category !== "all" && (
                <a
                  href={buildUrl('/skills', { locale, params: tag ? { tag } : undefined })}
                ...
                >
                  <span className="font-medium">Category:</span> {getCategoryDisplayName(category, locale)}
              ...
              {tag && (
                <a
                  href={buildUrl('/skills', { locale, params: category !== "all" ? { category } : undefined })}
                  className="inline-flex items-center h-7 gap-1.5 pl-3 pr-2 text-xs rounded-md bg-secondary text-secondary-foreground hover:bg-secondary/80 transition-colors"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredSkills.map((skill) => (
              <a
                ...
                href={buildUrl(`/skills/${skill.metadata.name}`, { locale })}
                className="block"
              >
